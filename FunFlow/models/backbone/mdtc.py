...
class TCNBlock(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            cache: Input cache (B, D, self.padding)

        Returns:
            outputs: Output tensor (B, D, T)
            new_cache: New cache (B, D, self.padding)
        """
        if cache.size(0) == 0:
...
@BACKBONES.register("MDTC", force=True)
class MDTC(nn.Module):
    """Multi-scale Depthwise Temporal Convolution for speech processing."""
    ...
    def __init__(
        ...
        for i in range(stack_num):
            self.blocks.append(
                TCNStack(res_channels, stack_size, 1, res_channels, kernel_size, causal)
            )
